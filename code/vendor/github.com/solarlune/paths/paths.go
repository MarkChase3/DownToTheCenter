...
// getCellIfExists receives a cell pointer and return it if it's not null, else returns a full zero node
func getCellIfExists(node *Cell) *Cell {
	if node == nil {
...
func (m *Grid) GetPathFromCells(start, dest *Cell, diagonals, wallsBlockDiagonals bool) *Path {
	...
	for {
		...
		if diagonals {
			...
			up := getCellIfExists(m.Get(node.Cell.X, node.Cell.Y-1)).Walkable
			down := getCellIfExists(m.Get(node.Cell.X, node.Cell.Y+1)).Walkable
			left := getCellIfExists(m.Get(node.Cell.X-1, node.Cell.Y)).Walkable
			right := getCellIfExists(m.Get(node.Cell.X+1, node.Cell.Y)).Walkable
